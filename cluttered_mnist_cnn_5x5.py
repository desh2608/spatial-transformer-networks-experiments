...
y_conv=stable_softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

cross_entropy = tf.reduce_mean(-tf.reduce_sum(y * tf.log(tf.clip_by_value(y_conv,1e-10,1.0)), reduction_indices=[1]))
opter = tf.train.AdamOptimizer(1e-4)
train_step = opter.minimize(cross_entropy)
...
f = open('./results/cluttered_mnist_cnn_5x5.dat','w')
